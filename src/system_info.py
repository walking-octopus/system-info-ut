...
def getWaydroidInfo():
	...
	if waydroid_installed and waydroid_configured:
		...
		lineage_variant = "Vanilla" if 'VANILLA' in system_ota_config else "GApps"

		waydroid_config = waydroid.tools.config.session_defaults["config_path"]
		...
		ota_version = None
		lineage_variant = None
		vendor_variant = None
		system_ota_config = None
		vendor_ota_config = None

	return {
		"basics": {
			"installed": waydroid_installed,
			"configured": waydroid_configured,
			"version": waydroid_version,
		},
		"container": {
			"version": ota_version,
			"variant": lineage_variant,
